...
export function useMateriCRUD(id_course: string) {
    ...
    const handleCreate = async (data: MateriFormValues) => {
        try {
            ...
            formData.append("id_course", data.id_course.toString());

            // Handle konten based on type
            if (data.tipeKonten === "pdf" && data.konten instanceof File) {
                formData.append("konten", data.konten);
            } else if (data.tipeKonten === "video" && typeof data.konten === "string") {
                formData.append("konten", data.konten);
            }
    ...
    const handleUpdate = async (
        ...
        try {
            ...
            if (data.tipeKonten === "pdf" && data.konten instanceof File) {
                formData.append("konten", data.konten);
            } else if (data.konten && typeof data.konten === "string") {
                formData.append("konten", data.konten);
            }
